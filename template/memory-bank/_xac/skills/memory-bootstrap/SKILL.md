...
Installing XAC into a project with two years of history leaves an empty bank that nobody
...
- Never bootstrap over an existing bank. That is `memory-migrate`. `memory-bank/_xac/` alone is
  not a bank; spine pages or family pages are.
