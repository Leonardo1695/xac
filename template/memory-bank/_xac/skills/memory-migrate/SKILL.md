---
name: memory-migrate
description: Migrates any existing project memory into the XAC memory-bank schema. Asks where the source memory lives first — XAC-style paths or elsewhere — then inventories, maps into families, and applies only with approval. Use when asked to migrate, adopt, or upgrade existing memory, or when XAC setup finds memory in another place or layout.
...
XAC's own files from an older install — `.cursor/rules/`, `.cursor/skills/`, parked `.new`
files — are not memory. `memory-bank/_xac/SETUP.md` retires them; this skill does not.
...
`_pending/`, `_lint/`, and spine files that are still missing, from
`memory-bank/_xac/templates/`. Create only what is missing. Touch no existing content yet.
...
what was found, the approved mapping, what was created or moved, and what is still
unresolved. Append a `log.md` line with the `migrated` event.
