...
The audit covers pages only: `memory-bank/_xac/` is XAC's own and is never a finding.
...
When a `rules/` page has proved itself across several sessions, propose copying it into the
project's own part of `AGENTS.md`, outside the XAC section. Say what it would add and why.
Never edit `AGENTS.md` unprompted — it loads every session, so every added line is a permanent
cost the user should agree to. Never add it inside the XAC section; upgrades replace that.
