...
The read protocol in `memory-bank.mdc` says what to load when a session opens. This is the
...
```powershell
rg -i "^entities:.*<noun>" memory-bank --glob "!**/_templates/**" --glob "!**/sessions/**" --glob "!**/tickets/archive/**"
```
...
**4. Full text, last.** Search the bodies only when the first three come up short. Exclude the
page templates and the never-loaded tier unless the task is about history:

```powershell
rg -i "<phrase>" memory-bank --glob "!**/_templates/**" --glob "!**/sessions/**" --glob "!**/tickets/backlog/**" --glob "!**/tickets/archive/**" --glob "!**/_pending/**" --glob "!**/_lint/**" --glob "!**/log.md"
```
