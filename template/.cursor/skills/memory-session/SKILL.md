...
If the memory bank is missing or the spine is empty, say so and offer to initialise from
`memory-bank/_templates/`. Fill it from the repo and the user's answers, never from guesses.
...
1. **Session page** — `memory-bank/sessions/<YYYY-MM-DD>-<short-slug>.md` from
   `_templates/session.md`. Fill Worked on, Outcome, Open, and Candidates. Candidates are
